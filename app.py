...
with left:
    ...
    with mc1:
        wr_adj = sc.williams_r + 100  # -100~0 → 0~100
        st.markdown(f"""
        <div class='card-sm'>
...
with right:
    ...
    cats = [
        ("추세 (Minervini+Weinstein+PTJ)", sc.trend_score, 30),
        ("모멘텀 (RSI+RS+MACD)", sc.momentum_score, 25),
        ("패턴 (VCP+Cup+Livermore)", sc.pattern_score, 20),
        ("수급/거래량 (MFI+Vol)", sc.flow_score, 15),
    ...
    <div class='card'>
      ...
      <div class='kv'>
        <div class='k'>Cup-with-Handle</div>
        <div class='v' style='color:{"#06b6d4" if cph["breakout"] else "#5a7299"};'>{cph['pattern']}</div>
      </div>
      <div class='kv'>
        <div class='k'>컵 깊이</div>
        <div class='v'>{cph['cup_depth_pct']}% {"✅" if 12<=cph['cup_depth_pct']<=35 else "⚠️"}</div>
      </div>
      <div class='kv'>
        <div class='k'>거래량 배수</div>
        <div class='v' style='color:{"#22c55e" if cph["vol_expansion"]>=1.5 else "#5a7299"};'>{cph['vol_expansion']}x</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class='card'>
      ...
      <div class='kv'>
        <div class='k'>지지선</div>
        <div class='v green'>{money(pivs.get('support', 0))}</div>
      </div>
      <div class='kv'>
        <div class='k'>저항선 (피벗)</div>
        <div class='v red'>{money(pivs.get('resistance', 0))}</div>
      </div>
      <div class='kv'>
        <div class='k'>피벗 돌파 임박</div>
        <div class='v' style='color:{"#06b6d4" if pivs.get("near_breakout") else "#5a7299"};'>
          {"🎯 임박!" if pivs.get("near_breakout") else "—"}
        </div>
      </div>
      <div class='kv'>
        <div class='k'>VCP 수축율</div>
        <div class='v'>{sc.vcp['tightness']*100:.0f}%</div>
      </div>
      <div class='kv'>
        <div class='k'>골든크로스</div>
        <div class='v' style='color:{"#22c55e" if sc.golden_cross else "#5a7299"};'>
          {"🌟 발생" if sc.golden_cross else "—"}
        </div>
      </div>
      <div class='kv'>
        <div class='k'>데드크로스</div>
        <div class='v' style='color:{"#ef4444" if sc.death_cross else "#5a7299"};'>
          {"💀 발생" if sc.death_cross else "—"}
        </div>
      </div>
      <div class='kv'>
        <div class='k'>52주 위치</div>
        <div class='v'>{sc.week52_pos*100:.0f}% (저: {money(sc.week52_l)} / 고: {money(sc.week52_h)})</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
#  멀티 티커 스캔 (Minervini 방식 — 시장 리더 선별)
# ══════════════════════════════════════════════════════════════════
st.markdown("---")
with st.expander("⚡ 멀티 티커 스캔 — Minervini식 시장 리더 선별"):
    sc1, sc2, sc3 = st.columns([3, 1, 1])
    with sc1:
        tickers_raw = st.text_area(
            "티커 목록",
            value="NVDA,TSLA,MSFT,AAPL,AMZN,META,GOOG,PLTR,SMR,IREN,AXON,CRWD,MSTR,SNOW,NET",
            height=70, label_visibility="collapsed")
    with sc2:
        scan_tf = st.selectbox("TF", list(TF_OPTIONS.keys()), key="scan_tf")
    with sc3:
        scan_style = st.selectbox("스타일", ["스윙","단타"], key="scan_style")

    tickers_list = [t.strip().upper() for t in tickers_raw.replace("\n",",").split(",") if t.strip()][:25]

    if st.button("🚀 스캔 시작 (Minervini TT 필터)", key="scan_btn"):
        rows = []
        prog = st.progress(0)
        for i, t in enumerate(tickers_list):
            prog.progress((i+1)/len(tickers_list), text=f"분석: {t}")
            res = build_signal(t, scan_style, scan_tf)
            if not res: continue
            s, d = res
            tp = s.trade_plan
            rows.append({
                "티커": t,
                "현재가": f"${tp.entry_price:,.2f}",
                "점수": s.sc.total,
                "등급": s.sc.grade,
                "시그널": s.action,
                "TT": f"{s.sc.trend_template['passed']}/8",
                "Stage": s.sc.weinstein["stage"],
                "RS": s.sc.rs["rs_score"],
                "진입가": f"${tp.entry_price:.2f}",
                "손절가": f"${tp.stop_price:.2f}({tp.stop_pct:.1f}%)",
                "1차TP": f"${tp.tp1_price:.2f}({tp.tp1_pct:+.1f}%)",
                "2차TP": f"${tp.tp2_price:.2f}({tp.tp2_pct:+.1f}%)",
                "3차TP": f"${tp.tp3_price:.2f}({tp.tp3_pct:+.1f}%)",
                "가중R:R": f"{tp.weighted_rr:.1f}",
                "VCP": "✅" if s.sc.vcp["breakout_ready"] else ("⚡" if s.sc.vcp["detected"] else "—"),
                "RSI": f"{s.sc.rsi_val:.0f}",
            })
        prog.empty()
        if rows:
            out = pd.DataFrame(rows).sort_values("점수", ascending=False)
            st.dataframe(
            ...
            # 상위 리더 하이라이트
            leaders = out[out["Stage"].astype(str) == "2"].head(5)
            if not leaders.empty:
                st.success(f"🌟 Minervini Stage 2 리더 종목: {', '.join(leaders['티커'].tolist())}")
        else:
            st.info("스캔 결과가 없어요.")
